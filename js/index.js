...
$.get(url1+"access_token="+access_token, function(data2, status){
 ...
 $.each(data2.data, function( index, value ) {      
   var row = $("<tr><td>&emsp;&emsp;ID:&emsp;"+value.id + "</td><td>&emsp;&emsp;Created time:&emsp; "+value.created_time+ "</td><td>&emsp;&emsp;Content:&emsp;"+value.message + "</td><a onclick=deletePost('+index.id+')><a>&emsp;&emsp;&emsp delete</a></tr>");
   $("#show").append(row);
 });
});
});
